...
class KeepAliveServer:
        ...
        try:
            ...
            async def handle_health(request):
                return web.json_response({
                    "status": "online",
                    "bot": bot.user.name if bot.user else "Conectando...",
                    "timestamp": datetime.now().isoformat()
                })
            
            self.app.router.add_get('/', handle)
            self.app.router.add_get('/health', handle_health)
            self.app.router.add_get('/ping', handle)
            
            self.runner = web.AppRunner(self.app)
            await self.runner.setup()
            
            # Usar a porta definida no Render ou 8080 como fallback
            self.site = web.TCPSite(self.runner, '0.0.0.0', port)
            await self.site.start()
            
            print(f"🌐 Keep-alive iniciado na porta {port}")
            print(f"✅ Servidor web rodando: http://0.0.0.0:{port}/")
            
            return True
            
        except Exception as e:
            print(f"⚠️ Não foi possível iniciar keep-alive: {type(e).__name__}: {e}")
    ...
    async def stop(self):
        ...
        try:
            ...
            print(f"⚠️ Erro ao parar servidor web: {e}")
...
async def main():
    ...
    # Iniciar keep-alive (opcional)
    try:
        print("🌐 Iniciando servidor keep-alive...")
        success = await keep_alive.start_simple()
        if not success:
            print("ℹ️ Bot rodará sem servidor web")
    except Exception as e:
        print(f"⚠️ Erro no keep-alive: {type(e).__name__}: {e}")
...
if __name__ == '__main__':
    # Configurar asyncio para evitar problemas no Render
